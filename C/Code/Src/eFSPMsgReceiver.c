        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    l_resultMsgD =  eFSP_MSGD_InitCtx(&p_ctx->msgDecoderCtnx, p_initData->i_memArea, p_initData->i_memAreaSize,
...
e_eFSP_MSGRX_Res eFSP_MSGRX_IsInit(s_eFSP_MSGRX_Ctx* const p_ctx, bool_t* p_isInit)
{
	...
	{
        l_resultMsgD = eFSP_MSGD_IsInit(&p_ctx->msgDecoderCtnx, p_isInit);
        l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
	}
...
e_eFSP_MSGRX_Res eFSP_MSGRX_NewMsg(s_eFSP_MSGRX_Ctx* const p_ctx)
{
	...
	else
	{
        ...
        else
        {
            ...
            l_resultMsgD = eFSP_MSGD_NewMsg(&p_ctx->msgDecoderCtnx);
            l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
...
e_eFSP_MSGRX_Res eFSP_MSGRX_GetDecodedData(s_eFSP_MSGRX_Ctx* const p_ctx, uint8_t** pp_data, uint32_t* const p_GetLen)
{
	...
	else
	{
		...
		else
		{
			...
			l_resultMsgD = eFSP_MSGD_GetDecodedData(&p_ctx->msgDecoderCtnx, pp_data, p_GetLen);
			l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
		}
	}
            ...
            while( l_stateM != MSGRX_PRV_ELABDONE )
            {
                switch( l_stateM )
                {
                    case MSGRX_PRV_CHECKINIT:
                    {
                        ...
                        l_isInit = false;
                        l_resultMsgD = eFSP_MSGD_IsInit(&p_ctx->msgDecoderCtnx, &l_isInit);
                        l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
                    ...
                    case MSGRX_PRV_CHECKINITTIMEOUT:
                    {
                        ...
                        if( true == p_ctx->rxTim.tim_getRemaining(p_ctx->rxTim.timerCtx, &l_sRemRxTime) )
                        {
                            /* Check also if we are still waiting start of frame to be received */
                            l_resultMsgD =  eFSP_MSGD_IsWaitingSof(&p_ctx->msgDecoderCtnx, &l_isWaitingSof);
                            l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
                    ...
                    case MSGRX_PRV_CHECKHOWMANYDATA:
                    {
                        ...
                        l_resultMsgD = eFSP_MSGD_GetMostEffDatLen(&p_ctx->msgDecoderCtnx, &l_rxMostEff);
                        l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
                        if( MSGRX_RES_OK == l_result )
                        {
                            if( l_rxMostEff > 0u )
                            ...
                            else
                            {
                                ...
                                l_resultMsgD = eFSP_MSGD_IsAFullMsgDecoded(&p_ctx->msgDecoderCtnx, &l_isMsgDec);
                                l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
                    ...
                    case MSGRX_PRV_RECEIVE_BUFF:
                    {
                        ...
                        if( true == (*p_ctx->cbRxP)(p_ctx->cbRxCtx, p_ctx->rxBuff, &p_ctx->rxBuffFill, l_rxMostEff,
                                                  l_cSessionRemanTime) )
                        {
                    ...
                    case MSGRX_PRV_INSERTCHUNK:
                    {
                        ...
                        if( l_cDToRxLen > 0u )
                        {
                            ...
                            * MSGD_InsEncChunk is well maden */
                            l_resultMsgD = eFSP_MSGD_InsEncChunk(&p_ctx->msgDecoderCtnx, cDToRxP, l_cDToRxLen, &l_cDRxed );
                            l_result = eFSP_MSGRX_convertReturnFromMSGD(l_resultMsgD);
                    ...
                    {
                        /* Check if frame timeout is eplased */
                        if( true == p_ctx->rxTim.tim_getRemaining(p_ctx->rxTim.timerCtx, &l_cRemainRxTime) )
                        {
                            /* Check time validity */
