...
typedef bool_t (*f_eFSP_MSGRX_RxCb) ( t_eFSP_MSGRX_RxCtx* const p_ptCtx, uint8_t* p_puDatRx, uint32_t* const p_puDatRxL,
                                      const uint32_t p_uCaxDataRxL, const uint32_t p_uTimeRx);

/* Call backs to functions that act as a timer */
typedef bool_t (*f_eFSP_MSGRX_TimStart) ( t_eFSP_MSGRX_TimCtx* const p_ptCtx, const uint32_t p_uTimeout );
typedef bool_t (*f_eFSP_MSGRX_TimGetRemaing) ( t_eFSP_MSGRX_TimCtx* const p_ptCtx, uint32_t* const p_puRemain );
...
typedef struct
{
    t_eFSP_MSGD_Ctx      tMsgdCtx;
	uint8_t*             puRxBuff;
	uint32_t             uRxBuffL;
	uint32_t             uRxBuffCntr;
    uint32_t             uRxBufFil;
    f_eFSP_MSGRX_RxCb    fRx;
    t_eFSP_MSGRX_RxCtx*  ptRxCtx;
    t_eFSP_MSGRX_Timer   tRxTim;
    uint32_t             uTimeoutMs;
    uint32_t             uTimePerRecMs;
    bool_t               bWaitFrmStart;
}t_eFSP_MSGRX_Ctx;
 ...
 *
 * @param[in]   p_ptCtx       - Msg receiver context
 * @param[out]  p_ppuD        - Pointer to a Pointer pointing to the decoded data payload ( NO CRC NO DATA SIZE )
 ...
 *              e_eFSP_MSGRX_RES_OK           	- Operation ended correctly
 */
e_eFSP_MSGRX_RES eFSP_MSGRX_GetDecodedData(t_eFSP_MSGRX_Ctx* const p_ptCtx, uint8_t** p_ppuD, uint32_t* const p_puGetL);
 ...
 *		    e_eFSP_MSGRX_RES_MESSAGERECEIVED  - Frame ended, restart context in order to parse a new frame. Every other
 *                                              call to this function will not have effect until we call
 *                                              eFSP_MSGRX_NewMsg, MSGRX_StartNewMsgNClean or timeout elapse. In this
 *                                              situation bear in mind that some data could be left out the parsing and
 *                                              can remain saved inside a RX buffer. The onlyway to completly clean RX
 *                                              buffer is calling MSGRX_StartNewMsgNClean.
 ...
 *                                              timeout elapse. In this situation bear in mind that some data could be
 *                                              left out the parsing and can remain saved inside a RX buffer.
 *          e_eFSP_MSGRX_RES_FRAMERESTART     - During frame receiving another start of frame is received. In this
 ...
 *                                              be left out the parsing and can remain saved inside a RX buffer.
 ...
 *          e_eFSP_MSGRX_RES_TIMCLBKERROR     - The timer function returned an error
 *          e_eFSP_MSGRX_RES_OK           	  - Operation ended correctly. The chunk is parsed correctly but the frame
 *                                              is not finished yet. This function return OK when the uITimePerRecMs
 *                                              timeout is reached, but uITimeoutMs is not elapsed.
 */
